  ...
  const fetchDocInfo = () => {
    if (doctors.length > 0 && docId) {
      const found = doctors.find(
        (doc) => doc._id.trim().toLowerCase() === docId.trim().toLowerCase()
      );
      if (found) {
        setDocInfo(found);
      }
      ...
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-800">
          ...
          <button
            ...
            className="bg-cyan-600 text-white text-sm font-light px-14 py-3 rounded-full my-6"
          >
            Book Your Appointment
          </button>
        </div>

        {/* Listing to Related Doctors */}
